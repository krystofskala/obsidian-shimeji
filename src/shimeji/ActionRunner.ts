...
export class ActionRunner {
	...
	private tickMove(frame: Frame, env: PushEnv, dt: number, ledges: Ledge[]): boolean {
		...
		// Which way this action's art was drawn travelling, and whether it is being used the other
		// way — see Mascot.directionalClimbArt for what acts on it.
		//
		// Read across the whole cycle rather than off the current pose. A climb routinely holds still
		// for a frame or two mid-cycle (the character this was reported on has a pose at zero), and
		// taking the sign from one of those said "no direction", dropped the mirror for exactly those
		// frames, and snapped the mascot upright and back — twenty-odd times on the way down one
		// wall. The direction belongs to the action, not to whichever frame happens to be showing.
		const authoredY = poses.reduce((sign, p) => sign || Math.sign(p.velocity?.y ?? 0), 0);
		const travellingY = targetY !== undefined ? Math.sign(targetY - physics.y) : 0;
		env.mascot.climbArtReversed = authoredY !== 0 && travellingY !== 0 && travellingY !== authoredY;
